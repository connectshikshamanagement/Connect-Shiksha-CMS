...
const fixProfitSharingCalculation = async () => {
  try {
    console.log('🔍 Fixing profit sharing calculation...');
    ...
    // Calculate correct profit sharing for ₹1,500 profit
    ...
    // Clear ALL existing payroll records for this project
    console.log('\n🗑️ Clearing ALL existing payroll records...');
    const deleteResult = await Payroll.deleteMany({ projectId: project._id });
    ...
    // Create NEW payroll records with correct amounts
    console.log('\n📝 Creating NEW payroll records...');
    ...
    console.log('\n🎉 Profit sharing calculation fixed!');
    console.log('\n💡 To prevent this issue in the future:');
    console.log('   - The profit sharing calculation should REPLACE existing records, not ADD to them');
    console.log('   - Consider updating the computeProjectProfitSharing function to clear existing records first');
    
    process.exit(0);
  } catch (err) {
    console.error('❌ Error fixing profit sharing calculation:', err);
    process.exit(1);
  }
};

fixProfitSharingCalculation();
